...
const projectStorage = localStorage.getItem('items')
...
function populateProjects() {
  ...
  newDiv.dataset.name =
    projectStorage[projectStorage.length - 1].dataset;

  let newButton = document.createElement('button');
  newButton.classList = 'list-button';
  newButton.dataset.name =
    projectStorage[projectStorage.length - 1].dataset;
  newButton.innerHTML =
    projectStorage[projectStorage.length - 1].title;
  newButton.style.background =
    projectStorage[projectStorage.length - 1].priority;
...
document
  ...
  .addEventListener('click', function() {
    document.querySelector('.projects-modal').style.display =
      'inline';
...
function clearProjectFields() {
  ...
  projectPriority = document.querySelector(
    '#project-priority',
  ).value = '';
}
  ...
  document.querySelectorAll('.list-button').forEach(function(e) {
    e.addEventListener('click', function(d) {
      ...
      document.querySelector('.modal-2-header').innerHTML =
        projectStorage[e.dataset.name].title;
      document.querySelector('.modal-2-header').style.background =
        projectStorage[e.dataset.name].priority;
